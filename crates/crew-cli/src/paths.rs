...
//!
//! `crew up` writes its PID to a file under the broker's state directory; `crew
//! down` reads it to find and signal the running unit. Both derive the path
//! from the same broker config, so they always agree.
