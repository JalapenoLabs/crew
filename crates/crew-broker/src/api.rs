...
use crate::{events, history, inbox};
...
/// `GET /inbox?role=<role>` (a role's live, self-filtered SSE stream), and
/// `GET /history` (read past events, filtered and paginated). The roster and the
