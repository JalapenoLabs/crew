...
- **Coordination robustness.** Parallel roles work in isolated git worktrees
  ...
  adversarial gate fails to break it; the defibrillator also catches coordination stalls,
  not just dead agents.
...
- **CLI (`crew`):** the operator front-end (`crew up` / `crew down`, and the
  ...
  `crew roster` / `crew lane`) for a runtime without MCP, and `crew watch` to tail a
...
`EventKind` (`Message` with a `MessageKind`, `Lifecycle`, `Activity`, `Boundary`) stream
...
API; it never touches the store. It exposes five
...
paths, and liveness), and `crew_lane` (check a path against the role's owned lane
...
issue #46). A tool failure returns as an `isError` result, not a protocol error. The
...
`crew inbox`, `crew roster`, and `crew lane` let an agent on a runtime without MCP, such
